...
class DottedDirectionsLine extends three_1.Mesh {
    constructor(points, params = {}) {
        ...
        this.geometry = new three_1.InstancedBufferGeometry();
        this.geometry.attributes = {
            position: new three_1.BufferAttribute(squarePosition, 3),
            uv: new three_1.BufferAttribute(squareUv, 2),
            instanceOffset: new three_1.InstancedBufferAttribute(new Float32Array(MAX_POINTS * 3), 3)
        };
        this.material = new three_1.RawShaderMaterial({
            uniforms: {
                color: { value: new three_1.Color(color) },
    ...
    setPoints(points) {
        ...
        const path = new three_1.Path(points);
        ...
        if (spacedPoints.length > attr.array.length / 3) {
            console.warn(`DottedDirectionsLine: too many points, max allowed` +
...
exports.default = DottedDirectionsLine;
